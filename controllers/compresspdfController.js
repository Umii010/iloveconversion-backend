...
const getMaxFiles = (req) => {
  ...
  if (req.session && req.session.userId) {
    // You need to implement this function based on your User model
    // This is a placeholder - you need to check your database
    const User = require('../models/User'); // Add this at the top
    
    // You could implement a middleware to check subscription
    // For now, we'll use a simple check
    return req.isProUser ? 999 : 15;
  }
  return 15; // Default for non-logged in users
};
...
exports.compressPdf = async (req, res) => {
  ...
  try {
    ...
    // Get dynamic max files based on user subscription
    const MAX_FILES = getMaxFiles(req);
    ...
    if (req.files.length > MAX_FILES) {
      ...
      const message = req.isProUser
        ? `Maximum ${MAX_FILES} files allowed per batch for Pro users. You uploaded ${req.files.length} files.`
        : `Maximum ${MAX_FILES} files allowed for free users. You uploaded ${req.files.length} files.`;
    ...
    // Add user subscription info to response headers
    res.set({
      'X-User-Is-Pro': req.isProUser ? 'true' : 'false',
      'X-Max-Files-Allowed': MAX_FILES.toString()
    });
    ...
    if (successful.length === 1) {
      ...
      res.set({
        ...
        'X-Is-Pro-User': req.isProUser ? 'true' : 'false'
      });
